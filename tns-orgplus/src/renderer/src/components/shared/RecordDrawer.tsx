...
export default function RecordDrawer({
  ...
  useEffect(() => {
    if (open && type === 'struttura' && record) {
      ...
      window.api.strutture.getDipendenti(s.codice).then(setStrutturaDipendenti)
    } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (mode === 'create') {
        if (type === 'struttura') {
          result = await window.api.strutture.create(form as Record<string, unknown>)
        } else {
          result = await window.api.dipendenti.create(form as Record<string, unknown>)
        }
      } else {
        if (type === 'struttura') {
          result = await window.api.strutture.update((r!.codice as string), form as Record<string, unknown>)
        } else {
          result = await window.api.dipendenti.update((r!.codice_fiscale as string), form as Record<string, unknown>)
        }
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      let result: { success: boolean; error?: string; message?: string }
      if (type === 'struttura') {
        result = await window.api.strutture.delete(r!.codice as string)
      } else {
        result = await window.api.dipendenti.delete(r!.codice_fiscale as string)
      }
      ...
      if (result.success) {
        ...
        setDeleteError(result.message ?? 'Impossibile eliminare')
      }
    } catch (e) {
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-6">
                ...
                {type === 'struttura' ? (
                  <>
                    <div>
                      ...
                      {fieldInput('codice', 'es. G01')}
                    </div>
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">Descrizione *</label>
                      {fieldInput('descrizione', 'Nome struttura')}
                    </div>
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">Struttura padre</label>
                      {fieldInput('codice_padre', 'es. A')}
                    </div>
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">CdC Costo</label>
                      {fieldInput('cdc_costo')}
                    </div>
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">Titolare</label>
                      {fieldInput('titolare', 'COGNOME NOME')}
                    </div>
                  </>
                ) : (
                  <>
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">Codice Fiscale *</label>
                      {fieldInput('codice_fiscale')}
                    </div>
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">Titolare</label>
                      {fieldInput('titolare', 'COGNOME NOME')}
                    ...
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">Struttura</label>
                      {fieldInput('codice_struttura', 'Codice struttura')}
                    </div>
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">CdC Costo</label>
                      {fieldInput('cdc_costo')}
                    ...
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">Unità Organizzativa</label>
                      {fieldInput('unita_organizzativa')}
                    </div>
                  </>
                )}
              </div>
          ...
          <div>
            ...
            {mode !== 'view' ? (
              ...
              <div>
                {ROLE_FIELDS.map(({ key, label }) => (
                  <RoleRow key={key} label={label} value={r?.[key] as string} />
                ))}
              </div>
            )}
          </div>
          ...
          <div>
            ...
            {mode !== 'view' ? (
              ...
              <div>
                {CLASSIFICAZIONI_FIELDS.map(({ key, label }) => (
                  <FieldRow key={key} label={label} value={r?.[key] as string} />
                ))}
              </div>
            )}
          </div>

          {/* Dipendenti section (struttura only, view mode) */}
          {type === 'struttura' && mode === 'view' && (
            <div>
              <div className="flex items-center justify-between mb-3">
                <p className={SECTION_LABEL + ' mb-0'}>
                  Dipendenti ({strutturaDipendenti.length})
                </p>
              </div>

...
export default function GridView() {
  ...
  // ── Helper: salva una singola cella ─────────────────────────────
  ...
  // ── Inline cell editing handler (propaga a tutte le righe selezionate) ──
  const handleCellValueChanged = useCallback(
    async (params: CellValueChangedEvent) => {
      ...
      // Righe selezionate esclusa quella già modificata
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 bg-white border-b border-gray-200">
        ...
        <div className="flex gap-1 flex-wrap">
          ...
          >
            ⚠️ Orfani Dip.{orphanDipendenti.length > 0 && (
          ...
          >
            ⚠️ Orfani Str.{orphanStrutture.length > 0 && (
              <span className="ml-1 bg-amber-500 text-white text-xs rounded-full px-1.5 py-0.5">
          ...
          >
            🌿 Str. Vuote{emptyStrutture.length > 0 && (
        ...
        {/* Sede filter — solo sui tab principali */}
        {isMainTab && (
          <select
            value={sedeFiltro}
        ...
        {/* Show deleted toggle — solo sui tab principali */}
        {isMainTab && (
          <button
        ...
        {/* Add button — solo sui tab principali */}
        {isMainTab && (
          <button
      ...
      {(subTab === 'orfani_dipendenti' || subTab === 'orfani_strutture' || subTab === 'strutture_vuote') && (
        <div className={[
          ...
          {subTab === 'orfani_dipendenti' && '⚠️ Dipendenti la cui struttura di assegnazione (codice_struttura) non esiste nel database. Doppio click su una cella per modificarla direttamente.'}
          {subTab === 'orfani_strutture' && '⚠️ Strutture il cui padre (codice_padre) non esiste nel database. Doppio click su una cella per correggere il riferimento.'}
          {subTab === 'strutture_vuote' && '🌿 Strutture senza dipendenti in nessun livello sottostante. Utile per identificare rami obsoleti dell\'organigramma.'}
        </div>
      )}
